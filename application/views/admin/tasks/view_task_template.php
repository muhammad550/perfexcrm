                ...
                <?php
                  ...
                  foreach ($task->comments as $comment) {
                      ...
                      if ($comment['file_id'] != 0) {
                        ...
                        $comments .= '<span class=""><a href="#" onclick="toggleReply(' . $comment['id'] . ',' . $task->id . '); return false;">Reply</span></a>';
                        echo form_open(admin_url('tasks/reply_comment'), ['id' => 'task-reply-comment-form', 'class' => '', 'style' => 'min-height:auto;background-color:#fff;']);
                        $comments .= '<div id="reply_comment_' . $comment['id'] . '" class="hide reply-task-comment" data-reply-comment="' . $comment['id'] . '">
                            <textarea rows="5" name="reply_comment" id="reply_task_comment_' . $comment['id'] . '" class="ays-ignore form-control">' . str_replace('[task_attachment]', '', 'String Reply') . '</textarea>;
                        <div class="clearfix mtop10"></div>
                        <button type="button" class="btn btn-primary pull-right" onclick="reply_comment(' . $comment['id'] . ',' . $task->id . ')">' . _l('submit') . '</button>
                        <button type="button" data-loading-text="loading"  class="btn btn-default pull-right mright5" onclick="cancel_toggle_reply('.$comment['id'].')">' . _l('cancel') . '</button>
                        </div>';
                        echo form_close();
                        $comments .= '</div>';
                      if ($i >= 0 && $i != $len - 1) {
...
function toggleReply(commentId, task_id) {
    var replyTextarea = document.getElementById('reply_comment_' + commentId);
    if (replyTextarea.classList.contains('hide')) {
        replyTextarea.classList.remove('hide');
    } else {
        replyTextarea.classList.add('hide');
    }
}
function cancel_toggle_reply(commentId){
    // alert(commentId);
    var replyTextarea = document.getElementById('reply_comment_' + commentId);
     if (replyTextarea.classList.contains('hide')) {
        replyTextarea.classList.remove('hide');
